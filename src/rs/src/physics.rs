        ...
        for i in 1..(nx - 1) {
            for j in 1..(ny - 1) {
                for k in 1..(nz - 1) {
                    ...
                    // Add the Qdot star from Technical report
                    // Qstar = (T_out - T_in) * A / (d/k + 1.0/hc_out)
                    // T_out is outside 
                    // T_in is GHOUSE 
                    // THIS SHOULD BE IN THE BOUNDARY CONDITION 
                    // CONDUCTIVE WALL
                   // if i == 1 && q {
                   //                    let t_in = p[[i, j, k]] / (globals::RS * p[[i, j, k]]);
                   //                    let t_out = 273.0_f32 + 90.0_f32;
                   //                    let vout = 4.0_f32;
                   //                    let hc_out = 10.0_f32 - vout + 10.0_f32 * vout.sqrt();
                   //                    let S = ds.powi(2);
                   //                 let d = 0.008_f32;
                   //                    let kappa = 0.2_f32;
                   //                    let Q = (t_out - t_in) * (S / ((d / kappa) + (1.0_f32 / hc_out)));
                   //                    energy_flux_x[[i, j, k, dim]] += Q / ds.powi(3);
                   //                }
